...
clean: ## Clean build artifacts
	python build_pypi_package.py clean

build: ## Build the package
	python build_pypi_package.py build

install: ## Install package locally (editable mode)
	python build_pypi_package.py install

install-dist: ## Install from built distribution
	python build_pypi_package.py install-dist

test: ## Test the installation
	python build_pypi_package.py test

check: ## Check package before upload
	python build_pypi_package.py check

upload-test: ## Upload to Test PyPI
	python build_pypi_package.py upload-test

upload: ## Upload to PyPI
	python build_pypi_package.py upload

all: ## Clean, build, check, and test
	python build_pypi_package.py all

publish: ## Clean, build, check, and upload to PyPI
	python build_pypi_package.py publish
